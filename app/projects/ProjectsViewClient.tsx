...
export default function ProjectsViewClient({ projects }: ProjectsViewClientProps) {
  ...
  const planningProjects = projects.filter(p => p.status === 'pending' || p.status === null).length;
  ...
  const filteredProjects = useMemo(() => {
    return projects.filter(project => {
      ...
      if (statusFilter !== 'all' && project.status !== statusFilter) {
        ...
        if (statusFilter === 'pending' && !project.status) return true;
        return false;
      }
      ...
      if (searchQuery.trim() !== '') {
        ...
        const titleMatch = project.project_name?.toLowerCase().includes(query);
        const locMatch = project.project_location?.toLowerCase().includes(query);
  ...
  const sortedProjects = useMemo(() => {
    return [...filteredProjects].sort((a, b) => {
      ...
      if (sortBy === 'deadline') {
        const dA = a.estimated_finish ? new Date(a.estimated_finish).getTime() : Infinity;
        const dB = b.estimated_finish ? new Date(b.estimated_finish).getTime() : Infinity;
        return dA - dB;
      }
      return 0;
    });
        ...
        <div className="flex w-full md:w-auto gap-4">
          ...
          <div className="w-full md:w-48 relative">
            ...
            >
              ...
              <option value="on_progress">Aktif (On Progress)</option>
