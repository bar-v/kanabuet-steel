...
export default function ProjectCard({ project }: { project: any }) {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case 'on_progress': return 'bg-brand-primary/10 text-brand-primary border border-brand-primary/20';
    ...
    <div className="flex flex-col bg-brand-card border border-brand-border rounded-2xl p-5 shadow-sm hover:shadow-[0_0_20px_rgba(234,88,12,0.1)] hover:border-brand-primary/40 hover:-translate-y-1 transition-all duration-300 relative group h-full">
      ...
      <div className="flex justify-between items-start gap-4 mb-3">
        <div className="flex-1">
          <h2 className="text-lg font-bold text-brand-text line-clamp-1 group-hover:text-brand-accent transition-colors" title={project.project_name}>
      ...
      <div className="mt-auto grid grid-cols-2 gap-3 text-sm border-t border-brand-border pt-4 mb-4">
        ...
        <div>
          ...
          <p className="text-brand-text font-medium text-sm">{formatDate(project.estimated_finish)}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 p-3 bg-brand-bg rounded-xl border border-brand-border">
        ...
        <span className="text-sm font-medium text-brand-subtext truncate" title={project.project_location || 'Lokasi Belum Ditentukan'}>
          {project.project_location || 'Lokasi Belum Ditentukan'}
        </span>
      </div>
